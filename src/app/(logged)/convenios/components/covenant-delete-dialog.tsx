...
interface CovenantDeleteDialogProps {
  id: string
  open: boolean
...
export function CovenantDeleteDialog({
  children,
  setOpen,
  open,
  id,
}: CovenantDeleteDialogProps) {
  const queryClient = useQueryClient()

  const { mutateAsync: deleteCovenantFn } = useMutation({
    mutationFn: async (id: string) => {
      await covenantServices.destroy(id)
    },
    onSuccess: () => {
      ...
      toast.success('Covenant excluído com sucesso!')
      setOpen(false)
    },
    onError: () => {
      toast.error('Erro ao excluir Covenant!')
    },
  })
