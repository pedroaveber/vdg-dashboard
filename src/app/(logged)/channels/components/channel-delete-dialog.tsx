...
export function ChannelDeleteDialog({
  ...
  const { mutateAsync: deleteChannelFn } = useMutation({
    mutationFn: async (id: string) => {
      await channelServices.destroy(id)
    },
    onSuccess: () => {
      queryClient.setQueryData<ChannelType[]>(['channels-general'], (prev) => {
        if (!prev) return []
